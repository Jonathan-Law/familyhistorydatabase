...
class mySession
{
   ...
   /**
     ...
     * getInstance return an istance of the class
   ...
   {
      if ($_MYSESSION_CONF['OVERWRITE_PHP_FUNCTION'] == 1)
      {
   ...
   private function readSessionId() 
   {
         ...
         { //there some jam in the cookie
            ...
            if ($this->checkSessionId()) 
            {
               ...
               if ($num != 1) 
               {
                  ...
                  if (!$this->newSid()) 
                  {
                     trigger_error("Unable to load session.1",E_USER_ERROR);
                  } 
                  else 
                  {
            ...
            else 
            {
               ...
               trigger_error("Unable to load session.2",E_USER_ERROR);
            }

         } 
         else 
         { 
            //dang it, no id ... i create some jam!

            if (!$this->newSid()) 
            {
               trigger_error("Unable to load session.3",E_USER_ERROR);
            } 
            else 
            {
               if (!$this->overwrite) setcookie ($this->sid_name, $this->sessionId,time()+$this->session_duration,"/",'',false,true);
      ...
      { 
      //no cookie allowed.. bad thing! search elsewhere

         if (isset($_REQUEST[$this->sid_name])) 
         {
         //bingo!
            ...
            else 
            {
               ...
               //jam .. maybe uncle tom eated it before ..
               $this->destroySession(FALSE);
               trigger_error("Unable to load session.4",E_USER_ERROR);
            }
   ...
   private function newSid() 
   {
      ...
      $fExpire = $this->forcedExpire;
      $ssid = $this->sessionId;
      $gua = $this->getUa();

      $this->SQLStatement_InsertSession->bindParam(':expires', $expireTime, PDO::PARAM_INT);
      $this->SQLStatement_InsertSession->bindParam(':forcedExpires', $fExpire, PDO::PARAM_INT);
      $this->SQLStatement_InsertSession->bindParam(':sid', $ssid, PDO::PARAM_STR, $this->sid_len);
      $this->SQLStatement_InsertSession->bindParam(':ua', $gua, PDO::PARAM_STR, 40);
   ...
   private function dbConnection() 
   {
      ...
      {
         echo $e->getMessage();
         die("There is an error with the connection");
      }
   }
...
//--------------------OVERWRITED FUNCTION
   ...
   {
      $tExpire = time() - $this->session_max_duration;
      $this->SQLStatement_DeleteExpiredSession->bindParam('time', $tExpire, PDO::PARAM_INT);
      ...
      $this->SQLStatement_DeleteExpiredSessionVars->bindParam('time', $tExpire, PDO::PARAM_INT);
      if ($this->SQLStatement_DeleteExpiredSessionVars->execute()===FALSE) 
      {
         trigger_error("Somenthing goes wrong with the garbace collector", E_USER_ERROR);
      } 
      else
...
//--------------------SQL FUNCTION
   ...
   private function setupSQLStatement() 
   {
      ...
      $this->SQLStatement_CountSid = 
      $this->connessione->prepare("SELECT count(*) FROM "
         .$tabella_sessioni." WHERE ".$this->table_column_sid." = :sid");

      /*** SQL statement: Insert Session ***/
      $this->SQLStatement_InsertSession = 
      $this->connessione->prepare("INSERT INTO "
         .$tabella_sessioni."(".$this->table_column_sid.",".$this->table_column_exp
            .",".$this->table_column_fexp.",".$this->table_column_ua
            .") VALUES (:sid,:expires,:forcedExpires,:ua)");

      /*** SQL statement: Update Session Expires ***/
      $this->SQLStatement_UpdateSessionExpires = 
      $this->connessione->prepare("UPDATE ".$tabella_sessioni
         ." SET ".$this->table_column_exp." = :expires WHERE "
         .$this->table_column_sid." = :sid");

      /*** SQL statement: Get Session Infos ***/
      $this->SQLStatement_GetSessionInfos = 
      $this->connessione->prepare("SELECT * FROM ".$tabella_sessioni
         ." WHERE ".$this->table_column_sid." = :sid");

      /*** SQL statement: Get Session Vars ***/
      $this->SQLStatement_GetSessionVars = 
      $this->connessione->prepare("SELECT ".$this->table_column_value
         ." as value, ".$this->table_column_name." as name FROM ".$tabella_variabili
         ." WHERE ".$this->table_column_sid
         ." = :sid");

      /*** SQL statement: Get Encrypted Session Vars ***/
      $this->SQLStatement_GetEncryptedSessionVars = 
      $this->connessione->prepare("SELECT AES_DECRYPT(".$this->table_column_value
         .",'".$this->encrypt_key."') as value,AES_DECRYPT("
         .$this->table_column_name.",'".$this->encrypt_key."') as name FROM "
      .$tabella_variabili." WHERE ".$this->table_column_sid
      ." = :sid");

      /*** SQL statement: Delete Session Vars ***/
      $this->SQLStatement_DeleteSessionVars = 
      $this->connessione->prepare("DELETE FROM ".$tabella_variabili
         ."  WHERE ".$this->table_column_sid." = :sid AND "
         .$this->table_column_name."= :name ");

      /*** SQL statement: Delete Encrypted Session Vars ***/
      $this->SQLStatement_DeleteEncryptedSessionVars = 
      $this->connessione->prepare("DELETE FROM ".$tabella_variabili
         ."  WHERE ".$this->table_column_sid." = :sid AND "
         .$this->table_column_name."= AES_ENCRYPT(:name,'".$this->encrypt_key."') ");

      /*** SQL statement: Insert Session Vars ***/
      $this->SQLStatement_InsertSessionVars = 
      $this->connessione->prepare("INSERT INTO ".$tabella_variabili."("
         .$this->table_column_sid.",".$this->table_column_name.","
         .$this->table_column_value.") VALUE(:sid,:name,:value) ON DUPLICATE KEY UPDATE ".$this->table_column_value." = :value");

      /*** SQL statement: Insert Encrypted Session Vars ***/
      $this->SQLStatement_InsertEncryptedSessionVars = 
      $this->connessione->prepare("INSERT INTO ".$tabella_variabili."("
         .$this->table_column_sid.",".$this->table_column_name.","
         .$this->table_column_value.") VALUE(:sid,AES_ENCRYPT(:name,'"
            .$this->encrypt_key."'),AES_ENCRYPT(:value,'".$this->encrypt_key."')) ON DUPLICATE KEY UPDATE ".$this->table_column_value." = AES_ENCRYPT(:value,'".$this->encrypt_key."')");

      /*** SQL statement: Insert Session Vars ***/
      $this->SQLStatement_UpdateSessionVars = 
      $this->connessione->prepare("UPDATE ".$tabella_variabili
         ." SET `value` = :value WHERE `sid` = :sid AND `name` = :name");

      /*** SQL statement: Insert Encrypted Session Vars ***/
      $this->SQLStatement_UpdateEncryptedSessionVars = 
      $this->connessione->prepare("UPDATE ".$tabella_variabili
         ." SET `value` = AES_ENCRYPT(:value,'"
            .$this->encrypt_key."') WHERE `sid` = :sid AND `name` = AES_ENCRYPT(:name,'"
            .$this->encrypt_key."')");

      /*** SQL statement: Delete Session ***/
      $this->SQLStatement_DeleteSession = 
      $this->connessione->prepare("DELETE FROM ".$tabella_sessioni
         ."  WHERE ".$this->table_column_sid." = :sid ");

      /*** SQL statement: Delete Expired Session ***/
      $this->SQLStatement_DeleteExpiredSession = 
      $this->connessione->prepare("DELETE FROM ".$tabella_sessioni
         ."  WHERE ".$this->table_column_fexp." < :time ");
      /*** SQL statement: Delete Expired Vars ***/
      $this->SQLStatement_DeleteExpiredSessionVars = 
      $this->connessione->prepare("DELETE FROM ".$tabella_variabili
         ."  WHERE ".$this->table_column_sid." NOT IN (SELECT ".$this->table_column_sid." FROM ".$tabella_sessioni.")");
   ...
   private function selectSessionVars() 
   {
      ...
      {
         // echo $this->SQLStatement_GetSessionVars->queryString;
   ...
   /**
     * Update current session var
     *
     * @access private     
     * @return boolean - True if the query succesfully done. False in any other case
     */
   private function updateSessionVar($name, $value) 
   {
      $this->SQLStatement_UpdateSessionExpires->bindParam(':expires', $newExprireTime, PDO::PARAM_INT);
      $this->SQLStatement_UpdateSessionExpires->bindParam(':sid', $this->sessionId, PDO::PARAM_STR, $this->sid_len);
      return $this->SQLStatement_UpdateSessionExpires->execute();
   ...
   /**
     * Send an update query to the DBMS
     *
     * Query will be created according to this prototype:
     * INSERT INTO $db.$tabelle SET $name = $value
     *
     ...
     * @return boolean - True if the query succesfully done. False in any other case
     */
   private function update($name,$val) 
   {
      if ($this->encrypt_data==1) 
      {
         $this->SQLStatement_UpdateEncryptedSessionVars->bindParam(':sid', $this->sessionId, PDO::PARAM_STR, $this->sid_len);
         $this->SQLStatement_UpdateEncryptedSessionVars->bindParam(':name', $name, PDO::PARAM_STR);
         $this->SQLStatement_UpdateEncryptedSessionVars->bindParam(':value', $val, PDO::PARAM_STR);
         $result = $this->SQLStatement_UpdateEncryptedSessionVars->execute();
      }
      else 
      {
         $this->SQLStatement_UpdateSessionVars->bindParam(':sid', $this->sessionId, PDO::PARAM_STR, $this->sid_len);
         $this->SQLStatement_UpdateSessionVars->bindParam(':name', $name, PDO::PARAM_STR);
         $this->SQLStatement_UpdateSessionVars->bindParam(':value', $val, PDO::PARAM_STR);
         $result = $this->SQLStatement_UpdateSessionVars->execute();
      }
      return $result;
   }
   ...
   /**
   *  Create the string version of the session
   */
   ...
   public function isExpired()
   {
      ...
      return ($val[0]['expires'] < time())? true:false;
   }
   public function login($user = null)
   {
      if($user)
      {
         $user->last_loggedin = mysql_time_stamp();
         $user->save();
   ...
   public function message($msg="")
   {
      ...
      {
         if (in_array('msg', self::$instance->VARS))
         {
