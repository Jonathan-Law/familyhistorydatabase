...
class Death
{
   ...
   public static function dropByPerson($temp_id = NULL)
   {
      ...
      if (isset($database))
      {
         return $database->SQLDelete('death', 'personId', $temp_id);
      }
   }
   ...
   protected function create()
   {
      ...
      if (isset($database))
      {
         ...
         // return $data;
         $insert = $database->SQLInsert($data, "death"); // return true if sucess or false
         if ($insert)
         {
            return $insert;
         }
   ...
   protected function update()
   {
      ...
      if (isset($database))
      {
         ...
         foreach($fields as $key)
         {
            $flag = $database->SQLUpdate("death", $key, $this->{$key}, "id", $this->id);
            if ($flag == "fail")
            {
               break;
            }
